...
int(timer_set_frequency)(uint8_t timer, uint32_t freq) {
  ...
  if (util_get_LSB(initial_value, &init_val_lsb))
    return 1;
  if (util_get_MSB(initial_value, &init_val_msb))
    return 1;

  if (sys_outb(initial_value, init_val_lsb))
    return 1;
  if (sys_outb(initial_value, init_val_msb))
    return 1;
